...
./linuxdeployqt appdir/starc -no-copy-copyright-files -no-translations -always-overwrite -no-strip -exclude-libs=libqsqlmimer,libqsqloci -extra-plugins=platforms,wayland-decoration-client,wayland-graphics-integration-client,wayland-shell-integration -executable=appdir/plugins/libcoreplugin.so -appimage -unsupported-allow-new-glibc
