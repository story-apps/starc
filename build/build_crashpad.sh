...
if [ "$UNIVERSAL_BUILD" = true ] && [ "$PLATFORM" = "mac" ]; then
    ...
    OUT_DIR_X64="out/${BUILD_MODE}_x64"
    mkdir -p "$OUT_DIR_X64"
    ...
    OUT_DIR_ARM64="out/${BUILD_MODE}_arm64"
    ...
    OUT_DIR="out/${BUILD_MODE}_universal"
    ...
    BUILD_DIR="$CRASHPAD_DIR/$OUT_DIR"
    TARGET_CPU="universal"
else
    # Обычная сборка для одной архитектуры
    echo "[*] Creating output directory..."
    OUT_DIR="out/${BUILD_MODE}_${TARGET_CPU}"
    mkdir -p "$OUT_DIR"
...
if [ "$PLATFORM" = "win" ]; then
    echo "    crashpad_handler: $BUILD_DIR/crashpad_handler.exe"
else
    echo "    crashpad_handler: $BUILD_DIR/crashpad_handler"
    echo "    Architecture: $TARGET_CPU"
fi

echo "[*] Copying crashpad libraries to starc libraries directory"
LIBS_DIR="$(pwd)/../src/_build/libs"
mkdir -p "$LIBS_DIR"

OBJDIR="$BUILD_DIR/obj"

# Определяем расширение библиотек в зависимости от платформы
if [ "$PLATFORM" = "win" ]; then
    LIB_EXT=".lib"
else
    LIB_EXT=".a"
...
# Список библиотек для копирования
# common и client из obj/client
# util из obj/util
# base из obj/third_party/mini_chromium/mini_chromium/base
# mig_output из obj/util (только для Mac)

# Функция для копирования библиотеки с проверкой разных вариантов именования
copy_lib() {
    local LIB_NAME=$1
    local LIB_PATH=$2
    local DEST_NAME=$3
    
    if [ -f "$OBJDIR/$LIB_PATH/lib${LIB_NAME}$LIB_EXT" ]; then
        cp "$OBJDIR/$LIB_PATH/lib${LIB_NAME}$LIB_EXT" "$LIBS_DIR/$DEST_NAME$LIB_EXT"
        echo "      $DEST_NAME$LIB_EXT"
        return 0
    elif [ "$PLATFORM" = "win" ] && [ -f "$OBJDIR/$LIB_PATH/${LIB_NAME}$LIB_EXT" ]; then
        cp "$OBJDIR/$LIB_PATH/${LIB_NAME}$LIB_EXT" "$LIBS_DIR/$DEST_NAME$LIB_EXT"
        echo "      $DEST_NAME$LIB_EXT"
        return 0
    else
        echo "      $DEST_NAME$LIB_EXT not found (checked: lib${LIB_NAME}$LIB_EXT, ${LIB_NAME}$LIB_EXT)"
        return 1
    fi
}

# Копируем библиотеки
copy_lib "common" "client" "libcommon"
copy_lib "client" "client" "libclient"
copy_lib "util" "util" "libutil"
copy_lib "base" "third_party/mini_chromium/mini_chromium/base" "libbase"
if [ "$PLATFORM" = "mac" ]; then
    copy_lib "mig_output" "util" "libmig_output"
fi

echo "    Libraries copied to: $LIBS_DIR"

# Копируем crashpad_handler в директорию сборки приложения
echo "[*] Copying crashpad_handler to starc build directory"
CRASHPAD_HANDLER_DIR="$(pwd)/../src/_build/crashpad"
mkdir -p "$CRASHPAD_HANDLER_DIR"

if [ "$PLATFORM" = "win" ]; then
    HANDLER_SRC="$BUILD_DIR/crashpad_handler.exe"
    HANDLER_DST="$CRASHPAD_HANDLER_DIR/crashpad_handler.exe"
else
    HANDLER_SRC="$BUILD_DIR/crashpad_handler"
    HANDLER_DST="$CRASHPAD_HANDLER_DIR/crashpad_handler"
fi
...
if [ -f "$HANDLER_SRC" ]; then
    ...
    echo "    crashpad_handler copied to: $CRASHPAD_HANDLER_DIR"
else
    echo "    crashpad_handler not found: $HANDLER_SRC"
    exit 1
fi
