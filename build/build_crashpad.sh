...
LIBS_DIR="$SCRIPT_DIR/../src/_build/libs"
mkdir -p "$LIBS_DIR"
...
# Функция для копирования библиотек
copy_lib() {
    local LIB_NAME=$1
    local LIB_PATH=$2
    
    if [ "$PLATFORM" = "win" ]; then
        FILE="${LIB_NAME}$LIB_EXT"
    else
        FILE="lib${LIB_NAME}$LIB_EXT"
    fi
    
    if [ -f "$OBJDIR/$LIB_PATH/$FILE" ]; then
        cp "$OBJDIR/$LIB_PATH/$FILE" "$LIBS_DIR/$FILE"
        echo "      $FILE"
        return 0
    elif [ "$PLATFORM" = "win" ] && [ -f "$OBJDIR/$LIB_PATH/$FILE" ]; then
        cp "$OBJDIR/$LIB_PATH/$FILE" "$LIBS_DIR/$FILE"
        echo "      $FILE"
        return 0
    else
...
copy_lib "common" "client"
copy_lib "client" "client"
copy_lib "util" "util"
copy_lib "base" "third_party/mini_chromium/mini_chromium/base"
if [ "$PLATFORM" = "mac" ]; then
    copy_lib "mig_output" "util"
fi
...
CRASHPAD_HANDLER_DIR="$SCRIPT_DIR/../src/_build/crashpad"
