...
class NovelOutlineSearchToolbar::Implementation
{
public:
     ...
     */
    void showPopup(NovelOutlineSearchToolbar* _parent);
...
NovelOutlineSearchToolbar::Implementation::Implementation(QWidget* _parent)
...
void NovelOutlineSearchToolbar::Implementation::showPopup(NovelOutlineSearchToolbar* _parent)
{
...
NovelOutlineSearchToolbar::NovelOutlineSearchToolbar(QWidget* _parent)
    ...
    , d(new Implementation(this))
{
    ...
    connect(d->goToPreviousAction, &QAction::triggered, this,
            &NovelOutlineSearchToolbar::findPreviousRequested);
    ...
    connect(d->goToNextAction, &QAction::triggered, this,
            &NovelOutlineSearchToolbar::findNextRequested);
    ...
    connect(d->matchCaseAction, &QAction::toggled, this,
            &NovelOutlineSearchToolbar::findTextRequested);
    //
    ...
    connect(d->replace, &Button::clicked, this, &NovelOutlineSearchToolbar::replaceOnePressed);
    addAction(d->replaceAllAction);
    connect(d->replaceAll, &Button::clicked, this, &NovelOutlineSearchToolbar::replaceAllPressed);
}

NovelOutlineSearchToolbar::~NovelOutlineSearchToolbar() = default;

void NovelOutlineSearchToolbar::setReadOnly(bool _readOnly)
{
...
void NovelOutlineSearchToolbar::refocus()
{
    d->searchText->setFocus();
}

QString NovelOutlineSearchToolbar::searchText() const
{
    return d->lastSearchText;
}

bool NovelOutlineSearchToolbar::isCaseSensitive() const
{
    return d->matchCaseAction->isChecked();
}

int NovelOutlineSearchToolbar::searchInType() const
{
    return d->popup->currentIndex().row();
}

QString NovelOutlineSearchToolbar::replaceText() const
{
    return d->replaceText->text();
}

bool NovelOutlineSearchToolbar::eventFilter(QObject* _watched, QEvent* _event)
{
    switch (_event->type()) {
    case QEvent::Resize: {
        if (_watched == parent()) {
...
bool NovelOutlineSearchToolbar::canAnimateHoverOut() const
{
    return !d->popup->isVisible();
}

void NovelOutlineSearchToolbar::processBackgroundColorChange()
{
...
void NovelOutlineSearchToolbar::processTextColorChange()
{
...
void NovelOutlineSearchToolbar::updateTranslations()
{
...
void NovelOutlineSearchToolbar::designSystemChangeEvent(DesignSystemChangeEvent* _event)
{
