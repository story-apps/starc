    ...
    const QModelIndexList& _indexes)
{
    ...
    connect(expandAll, &QAction::triggered, view, &Ui::ScreenplayTreatmentStructureView::expandAll);
    actions.append(expandAll);

    auto collapseAll = new QAction(tr("Collapse all"));
    collapseAll->setIconText(u8"\U000F0044");
    connect(collapseAll, &QAction::triggered, view,
            &Ui::ScreenplayTreatmentStructureView::collapseAll);
