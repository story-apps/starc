...
public:
     ...
     */
    void paintChapter(QPainter* _painter, const QStyleOptionViewItem& _option, const QModelIndex& _index) const;
...
void SimpleTextStructureDelegate::Implementation::paintChapter(QPainter* _painter, const QStyleOptionViewItem& _option, const QModelIndex& _index) const
{
    ...
    const qreal chapterHeadingWidth = backgroundRect.right()
                                      ...
                                      - chapterHeadingLeft - Ui::DesignSystem::treeOneLineItem().spacing();
    const QRectF chapterHeadingRect(QPointF(chapterHeadingLeft, backgroundRect.top() + Ui::DesignSystem::layout().px16()),
                                    QSizeF(chapterHeadingWidth, Ui::DesignSystem::layout().px24()));
    auto chapterHeading = _index.data(TextModelChapterItem::ChapterHeadingRole).toString();
    chapterHeading = _painter->fontMetrics().elidedText(
                         chapterHeading,
                         Qt::ElideRight,
                         static_cast<int>(chapterHeadingRect.width()));
    _painter->drawText(chapterHeadingRect, Qt::AlignLeft | Qt::AlignVCenter, chapterHeading);

    //
    // ... текст главы
    //
    auto chapterText = _index.data(TextModelChapterItem::ChapterTextRole).toString();
    ...
    if (textLines > 0) {
        ...
        const qreal chapterTextWidth = backgroundRect.right() - chapterTextLeft - Ui::DesignSystem::treeOneLineItem().margins().right();
        chapterTextRect = QRectF(QPointF(chapterTextLeft, chapterHeadingRect.bottom() + Ui::DesignSystem::layout().px8()),
                                 QSizeF(chapterTextWidth, _painter->fontMetrics().lineSpacing() * textLines));
        chapterText = TextHelper::elidedText(chapterText,
    ...
    // ... иконки заметок
    //
    const auto inlineNotesSize = _index.data(TextModelChapterItem::ChapterInlineNotesSizeRole).toInt();
    const qreal notesLeft = iconRect.left();
    const qreal notesTop = (chapterTextRect.isValid() ? chapterTextRect : chapterHeadingRect).bottom()
    ...
    const auto reviewMarksSize = _index.data(TextModelChapterItem::ChapterReviewMarksSizeRole).toInt();
...
QSize SimpleTextStructureDelegate::Implementation::chapterSizeHint(const QStyleOptionViewItem& _option, const QModelIndex& _index) const
{
    using namespace BusinessLayer;
    ...
    const bool haveNotesLine
            = (_index.data(TextModelChapterItem::ChapterInlineNotesSizeRole).toInt()
               + _index.data(TextModelChapterItem::ChapterReviewMarksSizeRole).toInt())
              > 0;
    if (haveNotesLine) {
...
void SimpleTextStructureDelegate::paint(QPainter* _painter, const QStyleOptionViewItem& _option, const QModelIndex& _index) const
{
    //
...
QSize SimpleTextStructureDelegate::sizeHint(const QStyleOptionViewItem& _option, const QModelIndex& _index) const
{
