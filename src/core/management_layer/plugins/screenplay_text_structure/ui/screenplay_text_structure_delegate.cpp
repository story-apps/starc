...
public:
    ...
    QSize beatSizeHint(const QStyleOptionViewItem& _option, const QModelIndex& _index) const;
    QSize textSizeHint(const QStyleOptionViewItem& _option) const;


    bool showFullBeatText = true;
...
void ScreenplayTextStructureDelegate::Implementation::paintBeat(QPainter* _painter,
                                                                ...
                                                                const QModelIndex& _index) const
{
    ...
    if (isLeftToRight) {
        headingLeft = iconRect.right() + Ui::DesignSystem::layout().px4();
        headingWidth
            = durationRect.left() - headingLeft - Ui::DesignSystem::treeOneLineItem().spacing();
    } else {
        headingLeft = durationRect.right() + Ui::DesignSystem::treeOneLineItem().spacing();
        headingWidth = iconRect.left() - headingLeft - Ui::DesignSystem::layout().px4();
    }
    auto sceneHeading
        = TextHelper::smartToUpper(_index.data(TextModelGroupItem::GroupHeadingRole).toString());
    const auto headingHeight = showFullBeatText
        ? TextHelper::heightForWidth(sceneHeading, _painter->font(), headingWidth)
        : Ui::DesignSystem::layout().px24();
    const auto headingTopDelta = showFullBeatText ? Ui::DesignSystem::layout().px(3) : 0.0;
    const auto headingTop
        = backgroundRect.top() + Ui::DesignSystem::layout().px16() + headingTopDelta;
    const QRectF headingRect(QPointF(headingLeft, headingTop), QSizeF(headingWidth, headingHeight));
    if (showFullBeatText) {
        _painter->drawText(headingRect, Qt::AlignLeft | Qt::AlignVCenter | Qt::TextWordWrap,
                           sceneHeading);
    } else {
        sceneHeading = _painter->fontMetrics().elidedText(sceneHeading, Qt::ElideRight,
                                                          static_cast<int>(headingRect.width()));
...
QSize ScreenplayTextStructureDelegate::Implementation::beatSizeHint(
    const QStyleOptionViewItem& _option, const QModelIndex& _index) const
{
    using namespace BusinessLayer;
    ...
    //
    // ... а затем вычитаем все возможные декорации
    //
    width -= Ui::DesignSystem::layout().px8() + Ui::DesignSystem::layout().px16()
        + TextHelper::fineTextWidthF("(00:00)", Ui::DesignSystem::font().body2())
        + Ui::DesignSystem::layout().px16();
    ...
    int height = Ui::DesignSystem::layout().px16() + Ui::DesignSystem::layout().px24();
    if (showFullBeatText) {
        height += TextHelper::heightForWidth(
            _index.data(TextModelGroupItem::GroupHeadingRole).toString(),
            Ui::DesignSystem::font().subtitle2(), width);
    }
    if (textLines > 0) {
        height += Ui::DesignSystem::layout().px8()
            + TextHelper::fineLineSpacing(Ui::DesignSystem::font().body2()) * textLines
...
void ScreenplayTextStructureDelegate::setShowFullBeatText(bool _showFull)
{
    d->showFullBeatText = _showFull;
}
                                                ...
                                                const QModelIndex& _index) const
{
    ...
    case TextModelItemType::Group: {
        ...
        if (groupType == TextGroupType::Scene) {
            ...
            return d->beatSizeHint(_option, _index);
        }
    }
