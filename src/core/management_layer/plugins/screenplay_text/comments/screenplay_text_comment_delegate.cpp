...
void ScreenplayTextCommentDelegate::paint(QPainter* _painter, const QStyleOptionViewItem& _option,
                                          const QModelIndex& _index) const
{
    ...
    _painter->fillRect(
        colorRect, _index.data(ScreenplayTextCommentsModel::ReviewMarkColorRole).value<QColor>());
    ...
    if (m_isSingleCommentMode || done) {
        ...
        iconRect = QRectF(
            QPointF(backgroundRect.right() - iconSize.width() - Ui::DesignSystem::layout().px12(),
                    backgroundRect.top() + Ui::DesignSystem::layout().px16()
    ...
    if (m_isSingleCommentMode || !done) {
        ...
        if (!comment.isEmpty()) {
            commentRect = QRectF(
                QPointF(avatarRect.left(), avatarRect.bottom() + Ui::DesignSystem::layout().px12()),
                QSizeF(commentWidth,
    ...
    if (!m_isSingleCommentMode && comments.size() > 1 && !done) {
        ...
        if (comments.size() > 2) {
            ...
            const QRectF commentsSummaryRect(
                avatarRect.right() + Ui::DesignSystem::layout().px12(), avatarRect.top(),
        ...
        QRectF lastCommentAvatarRect(
            QPointF(commentRect.left() + Ui::DesignSystem::layout().px12(),
        ...
        const auto maximumTextWidth = commentRect.right() - lastCommentAvatarRect.right()
        ...
        const QRectF lastCommentRect(lastCommentAvatarRect.right()
                                         + Ui::DesignSystem::layout().px12(),
        ...
        _painter->drawLine(
            QPointF(commentRect.left(), commentRect.bottom() + Ui::DesignSystem::layout().px16()),
            QPointF(commentRect.left(), lastCommentRect.bottom()));
        _painter->setOpacity(1.0);
    }
}

QSize ScreenplayTextCommentDelegate::sizeHint(const QStyleOptionViewItem& _option,
                                              const QModelIndex& _index) const
{
    //
    // Ширина
