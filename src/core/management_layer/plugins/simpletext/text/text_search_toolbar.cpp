...
class TextSearchToolbar::Implementation
{
public:
...
TextSearchToolbar::Implementation::Implementation(QWidget* _parent)
...
TextSearchToolbar::TextSearchToolbar(QWidget* _parent)
      ...
      d(new Implementation(this))
{
    ...
    connect(d->goToPreviousAction, &QAction::triggered, this, &TextSearchToolbar::findPreviousRequested);
    d->goToNextAction->setIconText(u8"\U000f0140");
    d->goToNextAction->setShortcut(QKeySequence::FindNext);
    addAction(d->goToNextAction);
    connect(d->goToNextAction, &QAction::triggered, this, &TextSearchToolbar::findNextRequested);
    ...
    connect(d->matchCaseAction, &QAction::toggled, this, &TextSearchToolbar::findTextRequested);

    addAction(d->replaceTextAction);
    addAction(d->replaceAction);
    connect(d->replace, &Button::clicked, this, &TextSearchToolbar::replaceOnePressed);
    addAction(d->replaceAllAction);
    connect(d->replaceAll, &Button::clicked, this, &TextSearchToolbar::replaceAllPressed);

    updateTranslations();
    designSystemChangeEvent(nullptr);
}

TextSearchToolbar::~TextSearchToolbar() = default;

void TextSearchToolbar::refocus()
{
    d->searchText->setFocus();
}

QString TextSearchToolbar::searchText() const
{
    return d->lastSearchText;
}

bool TextSearchToolbar::isCaseSensitive() const
{
    return d->matchCaseAction->isChecked();
}

QString TextSearchToolbar::replaceText() const
{
    return d->replaceText->text();
}

bool TextSearchToolbar::eventFilter(QObject* _watched, QEvent* _event)
{
    switch (_event->type()) {
        case QEvent::Resize: {
            if (_watched == parent()) {
...
void TextSearchToolbar::processBackgroundColorChange()
{
...
void TextSearchToolbar::processTextColorChange()
{
...
void TextSearchToolbar::updateTranslations()
{
...
void TextSearchToolbar::designSystemChangeEvent(DesignSystemChangeEvent* _event)
{
