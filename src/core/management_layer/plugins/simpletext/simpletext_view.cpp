...
#include "text/text_edit.h"
#include "text/text_edit_shortcuts_manager.h"
#include "text/text_edit_toolbar.h"
#include "text/text_search_manager.h"
...
public:
    ...
    TextEditShortcutsManager shortcutsManager;
    ScalableWrapper* scalableWrapper = nullptr;

    TextEditToolbar* toolbar = nullptr;
    BusinessLayer::TextSearchManager* searchManager = nullptr;
...
TextView::Implementation::Implementation(QWidget* _parent)
    : textEdit(new TextEdit(_parent)),
      shortcutsManager(textEdit),
      scalableWrapper(new ScalableWrapper(textEdit, _parent)),
      toolbar(new TextEditToolbar(scalableWrapper)),
      searchManager(new BusinessLayer::TextSearchManager(scalableWrapper, textEdit)),
...
void TextView::Implementation::updateToolBarUi()
{
...
void TextView::Implementation::updateToolBarCurrentParagraphTypeName()
{
      ...
      d(new Implementation(this))
{
    ...
    connect(d->toolbar, &TextEditToolbar::undoPressed, d->textEdit, &TextEdit::undo);
    connect(d->toolbar, &TextEditToolbar::redoPressed, d->textEdit, &TextEdit::redo);
    connect(d->toolbar, &TextEditToolbar::paragraphTypeChanged, this, [this] (const QModelIndex& _index) {
    ...
    connect(d->toolbar, &TextEditToolbar::searchPressed, this, [this] {
        d->toolbarAnimation->switchToolbars(d->toolbar->searchIcon(), d->toolbar->searchIconPosition(),
                                            d->toolbar, d->searchManager->toolbar());
    });
    //
    connect(d->searchManager, &BusinessLayer::TextSearchManager::hideToolbarRequested, this, [this] {
    ...
    connect(d->textEdit, &TextEdit::paragraphTypeChanged, this, handleCursorPositionChanged);
    connect(d->textEdit, &TextEdit::cursorPositionChanged, this, handleCursorPositionChanged);
...
void TextView::reconfigure(const QStringList& _changedSettingsKeys)
{
...
void TextView::loadViewSettings()
{
...
void TextView::saveViewSettings()
{
    using namespace DataStorageLayer;
...
QModelIndex TextView::currentModelIndex() const
{
    return d->textEdit->currentModelIndex();
}

void TextView::setCurrentModelIndex(const QModelIndex& _index)
{
    d->textEdit->setCurrentModelIndex(_index);
}

int TextView::cursorPosition() const
{
    return d->textEdit->textCursor().position();
}

void TextView::setCursorPosition(int _position)
{
