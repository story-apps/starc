    ...
    connect(
        ...
        [this, database = std::move(database), reportsToSend, dialog] {
            for (auto& report : reportsToSend) {
                ...
                loader->addRequestAttribute("key", QString("Qt") + QT_VERSION_STR);
                ...
                CrashpadPaths crashpadPaths;
                //
                // TODO: прикреплять лог файл сессии с крашем
                //
                const QString attachmentPath = q->logFilePath();
                if (QFile::exists(attachmentPath)) {
                    loader->addRequestAttributeFile("attachment.txt", attachmentPath);
                }
...
bool ApplicationManager::Implementation::initializeCrashpad()
{
    ...
    bool status = client->StartHandler(handler, reportsDir, metricsDir, url.toStdString(),
                                       annotations.toStdMap(), arguments, true, true, {});
    return status;
}
