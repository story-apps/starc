...
void ProjectManager::Implementation::updateViewsEditingMode()
{
    if (auto activeView = activeDocumentView(); activeView != nullptr) {
        ...
        if (view.active->currentDraft() > 0) {
            if (const auto draftIndex = view.active->currentDraft() - 1;
                item->drafts().size() > draftIndex) {
                              ...
                              const QString& _defaultMimeType)
{
    ...
    if (auto documentManager = d->pluginsBuilder.plugin(viewMimeType)->asQObject();
        ...
        if (documentManager->metaObject()->indexOfSignal("upgradeToCreatorRequested()")
            != invalidSignalIndex) {
            connect(documentManager, SIGNAL(upgradeToCreatorRequested()), this,
                    SIGNAL(upgradeToCreatorRequested()), Qt::UniqueConnection);
        }
