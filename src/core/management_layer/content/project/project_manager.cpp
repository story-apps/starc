...
void ProjectManager::Implementation::updateNavigatorContextMenu(const QModelIndex& _index)
{
    ...
    //
    // Если все выделенные итемы находятся в корзине, то их можно только удалить навсегда
    //
    bool allInRecycleBin = true;
    for (const auto index : selectedIndexes) {
        const auto currentItemIndex = projectStructureProxyModel->mapToSource(index);
        const auto currentItem = projectStructureModel->itemForIndex(currentItemIndex);
        if (!isInRecycleBin(currentItem)) {
            allInRecycleBin = false;
            break;
        }
    }
    if (allInRecycleBin) {
        ...
        connect(removeDocument, &QAction::triggered, q, [this] { removeSelectedDocuments(); });
        menuActions.append(removeDocument);
    } else {
        //
        // Иначе формируем список действий для конкретных элементов структуры проекта
        //
        ...
        //
            ...
            if (_index.isValid()) {
                ...
                if (addRemoveAction) {
                    auto removeDocument
                        = new QAction(selectedIndexes.size() > 1 ? tr("Remove documents")
                                                                 : tr("Remove document"));
