...
void ProjectsNavigator::designSystemChangeEvent(DesignSystemChangeEvent* _event)
{
    ...
    d->layout->setSpacing(DesignSystem::compactLayout().px16());
    d->layout->setContentsMargins(
        QMarginsF(DesignSystem::layout().px16(), DesignSystem::layout().px16(),
                  DesignSystem::layout().px16(), DesignSystem::layout().px16())
            .toMargins());
    const QMarginsF quoteMargins(DesignSystem::layout().px16(), DesignSystem::layout().px16(),
                                 DesignSystem::layout().px16(), 0.0);
    ...
    const QMarginsF quoteAuthorMargins(DesignSystem::layout().px16(), 0,
                                       DesignSystem::layout().px16(),
                                       DesignSystem::layout().px24() * 2);
