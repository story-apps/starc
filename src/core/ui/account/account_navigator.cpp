    ...
    , d(new Implementation(this))
{
    ...
    connect(d->renewSubscriptionButton, &Button::clicked, this,
            &AccountNavigator::renewSubscriptionPressed);
...
void AccountNavigator::updateTranslations()
{
    ...
    d->subscriptionAvailabilityLabel->setText(
        d->subscriptionEnd.isEmpty() ? tr("Information about subscription not loaded")
                                     : tr("Cloud service subscription available to") + " "
                + QDate::fromString(d->subscriptionEnd, "yyyy-MM-dd")
...
void AccountNavigator::designSystemChangeEvent(DesignSystemChangeEvent* _event)
{
    ...
    d->layout->setContentsMargins(
        QMarginsF(Ui::DesignSystem::layout().px2(), Ui::DesignSystem::layout().px2(),
                  Ui::DesignSystem::layout().px2(), Ui::DesignSystem::layout().px2())
            .toMargins());
    const QMarginsF titleMargins(Ui::DesignSystem::layout().px24(),
    ...
    d->subtitleLabel->setTextColor(ColorHelper::colorBetween(DesignSystem::color().primary(),
                                                             DesignSystem::color().onPrimary()));
    const QMarginsF cloudSpaceBarRect(0, Ui::DesignSystem::layout().px24(), 0,
