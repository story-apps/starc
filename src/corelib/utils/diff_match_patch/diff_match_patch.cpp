...
QString Diff::toString() const
{
    ...
    prettyText.replace('\n', QChar(L'\u00b6'));
    return QString("Diff(") + strOperation(operation) + QString(",\"") + prettyText
        + QString("\")");
}
...
{
    if (start1 == 0 && start2 == 0 && length1 == 0 && length2 == 0 && diffs.size() == 0) {
        return true;
    }
    return false;
}
...
QString Patch::toString()
{
    ...
    foreach (Diff aDiff, diffs) {
        ...
        text += QString(QUrl::toPercentEncoding(aDiff.text, " !~*();/?:@&=+$,#")) + QString("\n");
    }
                                           ...
                                           clock_t deadline)
{
    ...
    if (hm.count() > 0) {
        ...
        const QList<Diff> diffs_a = diff_main(text1_a, text2_a, checklines, deadline);
        const QList<Diff> diffs_b = diff_main(text1_b, text2_b, checklines, deadline);
        // Merge the results.
        diffs = diffs_a;
    ...
    while (thisDiff != NULL) {
        ...
        case EQUAL:
            ...
            if (count_delete >= 1 && count_insert >= 1) {
                ...
                foreach (Diff newDiff, diff_main(text_delete, text_insert, false, deadline)) {
...
QStringList diff_match_patch::diff_halfMatchI(const QString& longtext, const QString& shorttext,
                                              int i)
{
    ...
    if (best_common.length() * 2 >= longtext.length()) {
        QStringList listRet;
        listRet << best_longtext_a << best_longtext_b << best_shorttext_a << best_shorttext_b
                << best_common;
        return listRet;
    } else {
        return QStringList();
...
QRegularExpression diff_match_patch::BLANKLINESTART("^\\r?\\n\\r?\\n");
        ...
        case EQUAL:
            if (count_delete + count_insert > 1) {
                ...
                if (both_types) {
                    ...
                    if (commonlength != 0) {
                        thisDiff = &pointer.next();
                        thisDiff->text = safeMid(text_insert, text_insert.length() - commonlength)
                            + thisDiff->text;
        ...
        if (prevDiff->operation == EQUAL && nextDiff->operation == EQUAL) {
            ...
            if (thisDiff->text.endsWith(prevDiff->text)) {
                ...
                thisDiff->text = prevDiff->text
                    + thisDiff->text.left(thisDiff->text.length() - prevDiff->text.length());
                ...
                thisDiff->text = safeMid(thisDiff->text, nextDiff->text.length()) + nextDiff->text;
                pointer.remove(); // Delete nextDiff.
                nextDiff = pointer.hasNext() ? &pointer.next() : NULL;
                changes = true;
            }
...
QString diff_match_patch::diff_prettyHtml(const QList<Diff>& diffs)
{
    ...
    foreach (Diff aDiff, diffs) {
        ...
        switch (aDiff.operation) {
        case INSERT:
            html += QString("<ins style=\"background:#e6ffe6;\">") + text + QString("</ins>");
            break;
        case DELETE:
            html += QString("<del style=\"background:#ffe6e6;\">") + text + QString("</del>");
            break;
        case EQUAL:
...
double diff_match_patch::match_bitapScore(int e, int x, int loc, const QString& pattern)
{
...
QString diff_match_patch::patch_addPadding(QList<Patch>& patches)
{
    ...
    if (firstPatchDiffs.empty() || firstPatchDiffs.first().operation != EQUAL) {
        ...
        firstDiff.text
            = safeMid(nullPadding, firstDiff.text.length(), paddingLength - firstDiff.text.length())
            + firstDiff.text;
...
QList<Patch> diff_match_patch::patch_fromText(const QString& textline)
{
    ...
    Patch patch;
    QRegularExpression patchHeader(
        QRegularExpression::anchoredPattern("^@@ -(\\d+),?(\\d*) \\+(\\d+),?(\\d*) @@$"));
    char sign;
    QString line;
    while (!text.isEmpty()) {
        const auto match = patchHeader.match(text.front());
        if (!match.hasMatch()) {
