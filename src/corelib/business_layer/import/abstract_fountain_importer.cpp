...
QString AbstractFountainImporter::documentText(const QString& _text) const
{
    ...
    QStack<QString> dirs = processBlocks(paragraphs, writer);
    ...
    //
    while (!dirs.empty()) {
        processNotes({}, TextParagraphType::SequenceFooter, writer);
        dirs.pop();
    }
...
bool AbstractFountainImporter::isNotation() const
{
    return d->isNotation;
}

bool AbstractFountainImporter::isCommenting() const
{
    return d->isCommenting;
}

QStringList AbstractFountainImporter::sceneHeadingsDictionary() const
{
    return {
...
void AbstractFountainImporter::appendNotes(const QString& _paragraphText) const
{
    d->notes.append(std::make_tuple(_paragraphText, d->noteStartPos, d->noteLen));
    d->noteStartPos += d->noteLen;
    d->noteLen = 0;
}

void AbstractFountainImporter::processNotes(const QString& _paragraphText, TextParagraphType _type,
                                            QXmlStreamWriter& _writer) const
{
    ...
    for (int i = 0; i != _paragraphText.size(); ++i) {
        ...
        case '/': {
            if (prevSymbol == '*' && d->isCommenting) {
                ...
                //
                const bool shouldClosePrevBlock = !d->lastBlockText.isEmpty();
                appendBlock(d->blockText.left(d->blockText.size()), TextParagraphType::InlineNote,
                            _writer, shouldClosePrevBlock);
                d->blockText.clear();
            } else {
                if (d->isNotation) {
                    d->note.append('/');
        ...
        case '*': {
            if (prevSymbol == '/' && !d->isCommenting && !d->isNotation) {
                ...
                if (d->blockText.size() != 1) {
                    appendBlock(d->blockText.left(d->blockText.size() - 1), _type, _writer, true);
                    d->appendComments(_writer);
                    d->notes.clear();
                }
                d->blockText.clear();
        ...
        case ']': {
            if (prevSymbol == ']' && d->isNotation) {
                ...
                d->isNotation = false;
                d->notes.append(
                    std::make_tuple(d->note.left(d->note.size() - 1), d->noteStartPos, d->noteLen));
                d->noteStartPos += d->noteLen;
                d->noteLen = 0;
                d->note.clear();
            } else {
    ...
    if (!d->isNotation && !d->isCommenting) {
        ...
        if (!d->blockText.isEmpty() || _type == TextParagraphType::SequenceFooter) {
            const bool shouldClosePrevBlock = !d->isFirstBlock;
            appendBlock(d->blockText, _type, _writer, shouldClosePrevBlock);
        }
        d->blockText.clear();
    }
}

void AbstractFountainImporter::setCurrentBlockTextLast() const
{
    d->lastBlockText = d->blockText;

    //
    // Первый блок в тексте может встретиться лишь однажды
    //
    if (d->isFirstBlock) {
        d->isFirstBlock = false;
    }
}
