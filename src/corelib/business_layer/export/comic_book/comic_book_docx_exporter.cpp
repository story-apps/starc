 ...
 */
const ComicBookTemplate& exportTemplate(const ComicBookExportOptions& _exportOptions)
{
 ...
 */
bool needWriteHeader(const ComicBookExportOptions& _exportOptions)
{
 ...
 */
bool needWriteFooter(const ComicBookExportOptions& _exportOptions)
{
                 ...
                 const ComicBookExportOptions& _exportOptions)
{
    ...
    auto tableWidth = [&comicBookTemplate] {
        const QSizeF paperSize
            = QPageSize(comicBookTemplate.pageSizeId()).size(QPageSize::Millimeter);
        return mmToTwips(paperSize.width() - comicBookTemplate.pageMargins().left()
                         - comicBookTemplate.pageMargins().right());
    };
    ...
    //
        ...
        if (blockData != nullptr) {
            ...
            if (splitterItem->splitterType() == TextModelSplitterItemType::Start) {
                ...
                const int middleColumnWidth = pxToTwips(comicBookTemplate.pageSplitterWidth());
                const int leftColumnWidth = (fullTableWidth - middleColumnWidth)
                    * comicBookTemplate.leftHalfOfPageWidthPercents() / 100.;
...
void writeStaticData(QtZipWriter* _zip, const ComicBookExportOptions& _exportOptions)
{
    //
...
void writeStyles(QtZipWriter* _zip, const ComicBookExportOptions& _exportOptions)
{
    //
...
void writeHeader(QtZipWriter* _zip, const ComicBookExportOptions& _exportOptions)
{
    const auto& comicBookTemplate = exportTemplate(_exportOptions);
    ...
    const bool needPrintPageNumbers
        = comicBookTemplate.pageNumbersAlignment().testFlag(Qt::AlignTop);
    ...
    if (comicBookTemplate.pageNumbersAlignment().testFlag(Qt::AlignLeft) || !needPrintPageNumbers) {
        headerXml.append("left");
    } else if (comicBookTemplate.pageNumbersAlignment().testFlag(Qt::AlignCenter)) {
    ...
    if (comicBookTemplate.pageNumbersAlignment().testFlag(Qt::AlignRight)) {
...
void writeFooter(QtZipWriter* _zip, const ComicBookExportOptions& _exportOptions)
{
    const auto& comicBookTemplate = exportTemplate(_exportOptions);
    ...
    //
    const bool needPrintPageNumbers
        = comicBookTemplate.pageNumbersAlignment().testFlag(Qt::AlignBottom);
...
void writeDocument(QtZipWriter* _zip, ComicBookTextDocument* _comicBookText,
                   QMap<int, QStringList>& _comments, const ComicBookExportOptions& _exportOptions)
{
    QString documentXml
        = "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?>"
    ...
    //
    const auto& comicBookTemplate = exportTemplate(_exportOptions);
    QSizeF paperSize = QPageSize(comicBookTemplate.pageSizeId()).size(QPageSize::Millimeter);
    ...
    documentXml.append(QString("<w:pgMar w:left=\"%1\" w:right=\"%2\" w:top=\"%3\" w:bottom=\"%4\" "
                           ...
                           .arg(mmToTwips(comicBookTemplate.pageMargins().left()))
                           .arg(mmToTwips(comicBookTemplate.pageMargins().right()))
                           .arg(mmToTwips(comicBookTemplate.pageMargins().top()))
                           .arg(mmToTwips(comicBookTemplate.pageMargins().bottom()))
                           .arg(mmToTwips(comicBookTemplate.pageMargins().top() / 2))
                           .arg(mmToTwips(comicBookTemplate.pageMargins().bottom() / 2)));
    //
...
void ComicBookDocxExporter::exportTo(ComicBookTextModel* _model,
                                     const ComicBookExportOptions& _exportOptions) const
{
    ...
    if (zip.status() == QtZipWriter::NoError) {
        ...
        QScopedPointer<ComicBookTextDocument> document(prepareDocument(_model, _exportOptions));
