...
void StageplayFountainExporter::exportTo(AbstractModel* _model, ExportOptions& _exportOptions) const
{
    constexpr int invalidPosition = -1;
    exportTo(_model, invalidPosition, invalidPosition, _exportOptions);
}

void StageplayFountainExporter::exportTo(AbstractModel* _model, int _fromPosition, int _toPosition,
    ...
    while (block.isValid()) {
        ...
        if (!block.text().isEmpty()) {
                ...
                if (exportOptions.showBlockNumbers) {
                    ...
                    if (blockData != nullptr && blockData->item()->parent() != nullptr
                        ...
                        && static_cast<TextGroupType>(blockData->item()->parent()->subtype())
                            == TextGroupType::Scene) {
                        const auto sceneItem = static_cast<StageplayTextModelSceneItem*>(
            ...
            case TextParagraphType::Action: {
                ...
                paragraphText = TextHelper::toSentenceCase(paragraphText, capitalizeEveryWord,
                                                           capitalizeEverySentense);
                break;
            }
