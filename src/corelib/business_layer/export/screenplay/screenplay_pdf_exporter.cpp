    ...
    const QRectF& _blockRect, const QTextBlock& _block, const ExportOptions& _exportOptions) const
{
    ...
    auto printBlockDecoration = [_block, _painter, _blockRect](bool _isPrefix,
                                                               const QString& _text) {
        //
        ...
        //
        // Определим область для отрисовки декорации
        //
        ...
        if (_isPrefix) {
            ...
            decorationRect
                = QRectF(QPointF(prefixLeft, prefixTop), QSizeF(prefixWidth, prefixHeight));
        }
        //
        // ... постфикс
        //
            ...
            decorationRect
                = QRectF(QPointF(postfixLeft, postfixTop), QSizeF(postfixWidth, postfixHeight));
        }

        //
        // Собственно рисуем декорацию
        //
        _painter->drawText(decorationRect, Qt::AlignLeft | Qt::AlignBottom, _text);
    };
    ...
    //
        ...
        if (_block.blockFormat().boolProperty(TextBlockStyle::PropertyIsCharacterContinued)
            ...
            const auto isPrefix = false;
            printBlockDecoration(isPrefix, BusinessLayer::ScreenplayTextCorrector::continuedTerm());
        }
    }
    ...
    if (!_block.text().isEmpty()
        ...
        //
            ...
            //
            if (_block.charFormat().hasProperty(TextBlockStyle::PropertyPrefix)) {
                const auto isPrefix = true;
                printBlockDecoration(
                    isPrefix, _block.charFormat().stringProperty(TextBlockStyle::PropertyPrefix));
            }
            //
            // ... постфикс
            //
            if (_block.charFormat().hasProperty(TextBlockStyle::PropertyPostfix)) {
                const auto isPrefix = false;
                printBlockDecoration(
                    isPrefix, _block.charFormat().stringProperty(TextBlockStyle::PropertyPostfix));
            }
        }
    }
