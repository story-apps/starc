 ...
 */
enum class ScriptParagraphType {
    Undefined,
    UnformattedText,
    SceneName,
    SceneDescription,
 ...
 */
inline uint qHash(ScriptParagraphType _type)
{
...
public:
     ...
     */
    static ScriptParagraphType forBlock(const QTextBlock& _block);

public:
    ScriptBlockStyle() = default;

    /**
     * @brief Получить тип блока
     */
    ScriptParagraphType type() const;
     ...
     */
    void setType(ScriptParagraphType _type);
     ...
     */
    ScriptParagraphType embeddableFooter() const;

private:
    /**
     * @brief Инициилизация возможна только в классе стиля сценария
     */
    explicit ScriptBlockStyle(const QXmlStreamAttributes& _blockAttributes);
    friend class ScriptTemplate;
...
private:
     ...
     * @brief Тип блока
     */
    ScriptParagraphType m_type = ScriptParagraphType::Undefined;
...
public:
     ...
     */
    ScriptBlockStyle blockStyle(ScriptParagraphType _forType) const;

    /**
     * @brief Получить стиль заданного блока
     */
    ScriptBlockStyle blockStyle(const QTextBlock& _forBlock) const;
     ...
     */
    void setBlockStyle(const ScriptBlockStyle& _blockStyle);
...
private:
    explicit ScriptTemplate(const QString& _fromFile);
...
private:
     ...
     */
    QMap<ScriptParagraphType, ScriptBlockStyle> m_blockStyles;
};
