...
public:
    ...
    enum Property {
        ...
        PropertyCommentsIsAddition, //!< Список признаков того, что комментарий является добавлением
        PropertyCommentsIsRemoval, //!< Список признаков того, что комментарий является удалением
        PropertyCommentsIsEdited, //!< Список признаков изменений комментариев
        //
        // Свойства ресурсов
        //
        PropertyIsResourceMark, //!< Формат является пометкой ресурса
        PropertyResourceUuid, //!< Идентификаторо ресурса
        //
...
public:
    ...
    /**
     * @brief Экспортировать параграфы с горизонтальными отступами (true),
     *        или с пустыми линиями (false)
     * @note Необходимость появления этого парметра была обусловлена тем, что фильмтулз не умеет
     *       корректно считывать отступы у блоков при ручной вставке текста сцены, а только если в
     *       нём есть пустая строка, разделяющая блоки.
     *
     *       Так что это совершенно мусорный параметр, который 99.9% людей не нужен в нормальной
     *       жизни... поэтому его нужно выпилить это при первой же возможности.
     *
     *       При экспорте в DOCX, если задан этот параметр, то отступы вообще не учитываются, как бы
     *       они заданы не были, поэтому нужно следить, чтобы они были заданы именно строками
     */
    bool exportParagraphsWithMargins() const;
