...
{
    return m_device && ((m_position < m_buffer.size() - 1) || !m_device->atEnd());
}
    ...
    if (c == '{') {
        ...
        if ((c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z')) {
            ...
            if (m_has_value) {
                ...
                if (!ok || parsed > std::numeric_limits<qint32>::max()) {
                    throw tr("Invalid numeric parameter in RTF control word.");
                }
                m_value = static_cast<qint32>(parsed) * sign;
            }
            ...
            if (!ok) {
                throw tr("Invalid hexadecimal escape in RTF document.");
            }
            m_hex.append(static_cast<char>(value));
        } else {
