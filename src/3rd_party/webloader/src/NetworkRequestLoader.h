...
public:
    ...
    /**
     * @brief Загрузить список ссылок асинхронно, соединив возврат всех результатов с заданной
     * лямбдой
     */
    ...
    /**
     * @brief Загрузить список ссылок асинхронно, соединив возврат всех результатов с функцией
     * класса
     */
    template<typename Container, typename Func>
    static void loadAsync(const Container& _urlsList,
                          const typename QtPrivate::FunctionPointer<Func>::Object* _reciever = 0,
                          Func _slot = 0)
    {
        for (auto urlToLoad : _urlsList) {
